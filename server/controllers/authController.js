...
export async function initAdminPassword() {
  try {
    ...
    console.error(`[auth] init admin failed: ${msg}`);
  }
}
...
export async function login(req, res) {
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    return res.status(500).json({ ok: false, error: `login failed: ${msg}` });
  }
}
...
export async function changePassword(req, res) {
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    return res.status(500).json({ ok: false, error: `change password failed: ${msg}` });
  }
}
