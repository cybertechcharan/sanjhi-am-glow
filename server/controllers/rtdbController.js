...
async function ensureState() {
  const row = await RtdbState.findById("singleton").lean();
  if (row) return row;
  await RtdbState.create({ _id: "singleton", tree: {} });
  return { _id: "singleton", tree: {} };
}

function emitChange(req, path) {
  req.app.get("io")?.emit("rtdb:changed", { path, ts: Date.now() });
}
...
export async function setPath(req, res) {
  ...
  await RtdbState.updateOne({ _id: "singleton" }, { $set: { tree: nextTree } }, { upsert: true });
  emitChange(req, path);
